...
class SearchControllerTest(unittest.TestCase):
    ...
    def test_search_history_page_shows_saved_queries(self) -> None:
        ...
        fake_service = FakeSearchService(
            ...
            history_page=self._history_page(page=1, total_count=21),
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(fake_service.history_page_number, 1)
        self.assertEqual(fake_service.history_per_page, 20)
    ...
    def test_search_history_page_passes_page_number_to_service(self) -> None:
        """Номер страницы истории берется из query string и передается в SearchService."""
        fake_service = FakeSearchService(
            SearchResponseDTO(request_id=15, query_text="экономика", items=[]),
            history_page=self._history_page(page=2, total_count=35),
        )

        with patch("app.controllers.search_controller.SearchService", return_value=fake_service):
            response = self.client.get("/search/history?page=2")

        self.assertEqual(response.status_code, 200)
        self.assertEqual(fake_service.history_page_number, 2)
        self.assertEqual(fake_service.history_per_page, 20)
        self.assertIn("Страница 2", response.text)
        self.assertIn("page=1", response.text)

    def _history_page(self, *, page: int, total_count: int) -> SearchHistoryPageDTO:
        """Собрать тестовую страницу истории поиска."""
...
class FakeSearchService:
    ...
    history_page: SearchHistoryPageDTO | None = None
    last_query: str | None = None
    last_top_k: int | None = None
    saved_request_id: int | None = None
    history_page_number: int | None = None
    history_per_page: int | None = None
    ...
    def get_search_history(self, *, page: int, per_page: int) -> SearchHistoryPageDTO:
        """Запомнить параметры страницы и вернуть тестовую историю запросов."""
        self.history_page_number = page
        self.history_per_page = per_page
        return self.history_page or SearchHistoryPageDTO(
            items=[],
            page=page,
            per_page=per_page,
            total_count=0,
